...
function CustomCursor({
  ...
  // Gate the overlay on mount. `useReducedMotion` and the pointer media query
  // can only know the truth in the browser, so rendering the cursor during SSR
  // and then removing it on the client is a hydration mismatch (React throws
  // away the whole tree). Rendering nothing on the server and adding it after
  ...
  return (
    <CustomCursorContext.Provider value={contextValue}>
      <div
        ...
        {...props}
      >
        {!isMounted || isCoarsePointer || prefersReducedMotion ? null : (
          <motion.div
            animate={appearance}
            aria-hidden="true"
